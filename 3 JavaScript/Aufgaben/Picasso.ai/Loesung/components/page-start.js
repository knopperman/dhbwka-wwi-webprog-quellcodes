...
class PageStart extends HTMLElement {
    ...
    async connectedCallback() {
        if (!this.generating) {
            ...
            let allImages = await imageStore.getAllImages();

            let noImages = `
                <div id="no-images">
                    Es sind noch keine Bilder vorhanden. <br> Am besten generiere gleich ein neues.
                </div>
            `;
            ...
            let imageGallery = `
                <div id="gallery">
                    ${allImages.map((image, index) => `
                        <img src="${image.url}" data-index="${index + 1}" alt="${image.title}" class="panel">
                    `).join("")}
                </div>
            `;

            this.innerHTML = `
                ${allImages.length < 1 ? noImages : imageGallery}

                <div id="form" class="panel">
                    <input type="text" placeholder="Titel für neues Bild">
                    <button>Generieren</button>
                </div>
            `;

            for (let imageElement of this.querySelectorAll("img")) {
            ...
            this.querySelector("input").addEventListener("keyup", async event => {
                if (event.key === "Enter") await this.onGenerateClick();
            });

            this.querySelector("button").addEventListener("click", this.onGenerateClick.bind(this));
        } else {
